...
//        return ServerResponseVO.error(ServerResponseEnum.ERROR);
    ...
    /**
     * http://api.qingyunke.com/api.php?key=free&appid=0&msg=help
     * @param userInput
     * @return
     */
    public ServerResponseVO<UserInput> chatApi(UserInput userInput) {
        String userText = userInput.getText(); // 获取用户输入文本
        RestTemplate restTemplate = new MineRestTemplateBuilder().build();
        String url = "https://api.qingyunke.com/api.php?key=free&appid=0&msg=";
        url += userText;
//        Map var = new HashMap();
//        var.put("key", "free");
//        var.put("appid", "0");
//        var.put("msg", userText);
        ResponseEntity<Map> responseEntity = restTemplate.getForEntity(url, Map.class);
        String returnObj = (String)responseEntity.getBody().get("content");
        UserInput output = new UserInput();
        output.setResponse(returnObj);
        return ServerResponseVO.success(output);
    }
}
