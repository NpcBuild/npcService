...
public class JwtTokenUtil {
    ...
    public Claims getTokenClaim(String token) {
        try {
            ...
            e.printStackTrace();
            throw new YFLoginTimeoutException(ServerResponseEnum.TOKEN_INVALID.getMessage());
//            return null;
        }
    }
    ...
    public Boolean isTokenExpired(String token) {
        Boolean timeout = getExpirationDate(token).before(new Date());
        if (timeout) {
            throw new YFLoginTimeoutException(ServerResponseEnum.TOKEN_INVALID.getMessage());
        } else {
            return timeout;
